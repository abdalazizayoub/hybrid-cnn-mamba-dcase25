...
class FineTuneStudentModule(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.save_hyperparameters(config)
        self.config = config

        # 1. Initialize the 10-Class Student Model
        ...
        if hasattr(config, 'pretrained_ckpt') and config.pretrained_ckpt and os.path.exists(config.pretrained_ckpt):
            ...
            for k, v in pretrained_dict.items():
                ...
                if clean_k in student_dict and v.shape == student_dict[clean_k].shape:
                    filtered_dict[clean_k] = v
                else:
                    print(f" Dropping layer: {clean_k} (Shape mismatch - Initializing fresh)")
                    
            student_dict.update(filtered_dict)
            self.student.load_state_dict(student_dict)
            print("Backbone loaded! Fresh 10-class head ready for DCASE.\n")
        else:
    ...
    def configure_optimizers(self):
        # Using a Differential Learning Rate (Slightly slower for backbone, faster for the new head)
        backbone_params = []
        head_params = []
        ...
        optimizer = torch.optim.AdamW([
            ...
            {'params': head_params, 'lr': self.config.lr * 10.0}     
    ...
    def training_step(self, batch, batch_idx):
        x = batch[0]
        labels = batch[3] if len(batch) > 4 else batch[2]
...
def train(config):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        dirpath=ckpt_dir,
        filename='best-finetune-epoch={epoch:02d}-val_acc={val/macro_avg_acc:.2f}',
    ...
    pl_module = FineTuneStudentModule(config)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fine-Tune Pre-Trained Student on DCASE')
    parser.add_argument("--project_name", type=str, default="DCASE25_Hybrid_FineTune")
    parser.add_argument("--experiment_name", type=str, default="Hybrid_FineTune_25Percent")
    
    # --- PATH TO YOUR BEST PRE-TRAINED ESC-50 CHECKPOINT ---
    parser.add_argument("--pretrained_ckpt", type=str, required=True, help="Path to the best ESC-50 .ckpt file")
