...
class MultiDeviceModelContainer(nn.Module):
    ...
    def __init__(self, base_model: nn.Module, devices: list):
        ...
        # Create device-specific models (Clones your Hybrid Mamba perfectly)
    ...
    def forward(self, x: torch.Tensor, devices: Tuple[str] = None) -> torch.Tensor:
        ...
        """
        if devices is None:
            # No device info → use base model (Unseen fallback)
            ...
            # Single unknown device → fall back to base model (Unseen fallback)
            return self.base_model(x)
