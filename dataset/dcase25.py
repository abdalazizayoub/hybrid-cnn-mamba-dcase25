...
class DCASE25Dataset(Dataset):
    def __init__(self, meta_csv: str, roll_samples: int = 0, is_training: bool = False):
        ...
        # ==========================================
        # 🧠 1. The Student's Transform (Squashed Time, High Freq)
        # ==========================================
        self.student_transform = torchaudio.transforms.MelSpectrogram(
            sample_rate=44100,      
            n_fft=8192,             
            win_length=8192,        
            hop_length=1364,        
        ...
        # ==========================================
        # 🧑‍🏫 2. The Teacher's Transform (Standard AudioSet Format)
        # ==========================================
        self.teacher_transform = torchaudio.transforms.MelSpectrogram(
            sample_rate=32000,      
            n_fft=1024,             
            win_length=1024,        
            hop_length=320,         
    ...
    def __getitem__(self, index: int):
        ...
        # --- Generate the Student's Spectrogram ---
        x_student = self.student_transform(waveform)
        x_student = self.amp_to_db(x_student)

        # --- Generate the Teacher's Spectrogram ---
        waveform_32k = torchaudio.functional.resample(waveform, 44100, 32000)
        x_teacher = self.teacher_transform(waveform_32k)
        x_teacher = self.amp_to_db(x_teacher)
        
        # We now return 6 items instead of 5!
        return x_student, x_teacher, self.files[index], self.labels[index], self.devices[index], self.cities[index]
